...
export BAZEL_EXTRA_TEST_OPTIONS="--test_env=ENVOY_IP_TEST_VERSIONS=v4only --test_output=errors --remote_cache=${BAZEL_REMOTE_CACHE} --jobs=${NUM_CPUS}"

export BAZEL_BUILD_EXTRA_OPTIONS="--remote_cache=${BAZEL_REMOTE_CACHE} --jobs=${NUM_CPUS}"
