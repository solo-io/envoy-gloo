...
[[ -z "${GCOVR}" ]] && GCOVR=gcovr
# TODO(talnordan): [[ -z "${WORKSPACE}" ]] && WORKSPACE=envoy
[[ -z "${WORKSPACE}" ]] && WORKSPACE=envoy_gloo
...
# TODO(talnordan): SCRIPT_DIR="$(realpath "$(dirname "$0")")"
# TODO(talnordan): (BAZEL_BIN="${BAZEL_COVERAGE}" "${SCRIPT_DIR}"/coverage/gen_build.sh)
...
# run.
# TODO(talnordan): rm "${SRCDIR}"/test/coverage/BUILD
