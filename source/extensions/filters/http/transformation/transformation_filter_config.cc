...
Transformation::Transformation(const envoy::api::v2::filter::http::Transformation& transformation) : transformation_(transformation)  {
  switch (transformation_.transformation_type_case()) {
  case envoy::api::v2::filter::http::Transformation::kTransformationTemplate:
    passthrough_body_ = transformation_.transformation_template().has_passthrough();
    transformer_.reset(new InjaTransformer(transformation_.transformation_template()));
    break;
