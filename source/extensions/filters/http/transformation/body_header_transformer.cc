...
void BodyHeaderRequestTransformer::transform(
  ...
  /* auto json_body = extractBodyAndHeaders(request_headers, body, add_request_metadata_); */
  json json_body;
  if (body.length() > 0) {
  ...
  parse_headers(request_headers, headers, multi_value_headers);
...
void BodyHeaderRequestTransformer::parse_headers(
...
void BodyHeaderRequestTransformer::parse_query_string(
