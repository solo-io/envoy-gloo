...
struct PromptEnrichment {
  struct Message {
    Message(std::string role, std::string content)
        : role_(std::move(role)), content_(std::move(content)) {}
    virtual ~Message() = default;
    Message(const Message &other) = default; // Copy constructor
    Message &
    operator=(const Message &other) = default;     // Copy assignment operator
  ...
  PromptEnrichment &operator=(const PromptEnrichment &other) =
      default; // Copy assignment operator
  PromptEnrichment(PromptEnrichment &&other) = default; // Move constructor
  PromptEnrichment &
  operator=(PromptEnrichment &&other) = default; // Move assignment operator
...
private:
  ...
  template <typename... Args> void prependMessage(Args &&...args) {
    prepend_.emplace_back(std::forward<Args>(args)...);
  }

  template <typename... Args> void appendMessage(Args &&...args) {
    append_.emplace_back(std::forward<Args>(args)...);
  }
};
...
struct FieldDefault {
  ...
  FieldDefault &
  operator=(const FieldDefault &other) = default; // Copy assignment operator
  FieldDefault(FieldDefault &&other) = default;   // Move constructor
  FieldDefault &
  operator=(FieldDefault &&other) = default; // Move assignment operator
...
private:
  std::tuple<bool, bool>
  transformHeaders(Http::RequestHeaderMap *request_headers,
