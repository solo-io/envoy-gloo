...
public:
  ...
  bool body_passthrough() const { return passthrough_body_; }
...
public:
  ...
  const absl::optional<Transformation> &getRequestTranformation() const {
    return request_transformation_;
  }

  bool shouldClearCache() const { return clear_route_cache_; }

  const absl::optional<Transformation> &getResponseTranformation() const {
